...
Шрифты подключаются через `next/font` или `@font-face`: Playfair Display (weights 500/600/700) и Inter (weights 300/400/500/600).
...
Карточка hover: translateY(-6px), box-shadow → CSS transition, --duration-normal
...
```
- Inter 500, 15px, color `--text-primary` (или white, см. выше), letter-spacing 0
...
- H1 не должен самостоятельно масштабироваться или перестраиваться агентом по собственной инициативе.
- Не превращать H1 в обычный oversized text-overlay поверх видео.
...
- обязательный элемент Hero;
- не является обычным eyebrow.
...
Две CTA-кнопки образуют **одну компактную горизонтальную группу** и располагаются ближе к центральной области Hero.

- `Стать участником` — primary CTA.
- `Подробнее` — secondary CTA.
- Обе кнопки находятся на одной горизонтальной линии.
- Кнопки не разносить к противоположным краям экрана.
- Вертикально выровнять по одной оси.
- Между кнопками использовать небольшой фиксированный gap.
- CTA-группа располагается в центральной области Hero, примерно в зоне перехода glass/video.
- Обе кнопки используют **Liquid Glass visual language**.
- `Стать участником` — акцентный orange Liquid Glass вариант.
- `Подробнее` — прозрачный / светлый Liquid Glass вариант.
- Обе кнопки имеют одинаковую визуальную высоту и согласованные размеры.
- Использовать существующую реализацию `.btn-liquid` как основу; для вторичной кнопки использовать transparent/secondary variant, а не обычный `.btn-text`.
...
- H1 сохраняет разделение `Бизнес —` / `ЭКСПЕДИЦИЯ` для экспедиционных слайдов.
...
### 2.2 Market Reality (bento-статистика)

**Контейнер:** padding-top/bottom `--space-10` (96px) / `--space-8` mobile. Фон: `--bg-canvas`.

**Заголовок секции** (опционально, можно без): eyebrow + H2, text-align center, margin-bottom `--space-7` (40px).

**Grid:** 3 колонки desktop (gap 24px), 1 колонка mobile (gap 16px, stacked).

**Карточка статистики:**
- Фон `--bg-elevated`, border-radius `--radius-md`, padding 40px 32px
- border: 1px solid `--border-hairline`
- Цифра: `--type-stat`, margin-bottom 12px
- Подпись: `--type-body`, color `--text-secondary`, line-height 1.5
- Hover: box-shadow `--shadow-hover`, translateY(-4px), 300ms

Контент (реальный, с сайта): ">60% неудачных экспансий — из-за недостаточно глубокого анализа рынка", "50+ лояльных контактов — минимум для понимания специфики", ">40% стартапов терпят провал из-за невостребованности продукта".

### 2.3 Platform Statement

**Контейнер:** max-width 800px, margin: 0 auto, text-align center, padding `--space-9` 0 (64px).

**Типографика:**
- H2 (Playfair Display 600, 32px desktop / 24px mobile, line-height 1.3, color `--text-primary`)
...
- Подстрока Inter 400 16px `--text-secondary`: "Мы предлагаем авторские маршруты уникальных бизнес-экспедиций и собственный независимый дью-дилидженс."

**Декор:** тонкая горизонтальная линия `--orange-600`, width 64px, height 2px, margin: 24px auto (над заголовком).

**Три пилюли-метки** (под подстрокой, margin-top 32px):
```css
.platform-pills {
  display: flex;
...
Три пилюли (данные с сайта):
- "АНАЛИТИКА" / "рынка"
- "НАДЕЖНОСТЬ" / "партнеров"
- "ПРОВЕРКА" / "product-market fit"

**CTA под пилюлями:** `.btn-liquid`, текст "Участвовать в экспедиции", margin-top 8px.

### 2.4 Continental Navigation

**Заголовок:** eyebrow "РЫНКИ" + H2 "Выберите регион", margin-bottom 40px.

**Grid:** 4 колонки desktop (gap 24px), 2 колонки tablet, 1 колонка mobile (horizontal scroll snap вместо stack — см. мобильный раздел ниже).

**Карточка региона:**
- `.glass-01`, border-radius `--radius-lg`, padding 32px, aspect-ratio 4:3
- Фон карточки — полупрозрачный контур континента (SVG, `--continent-outline`, opacity .6) как декоративный слой позади текста
- H3 региона (Playfair Display 600, 22px) + количество направлений (`--type-meta`, напр. "3 направления")
- Hover: переключение на `.glass-02`, translateY(-4px)

### 2.5 Featured Markets (bento направлений)

**Grid:** асимметричный bento — 1 крупная карточка (featured, span 2 колонки × 2 строки) + остальные обычные (span 1×1), 3 колонки desktop, gap 24px. Mobile — stack, featured карточка первая.

**Featured-карточка:**
- Фон: `--gradient-signature`
- padding 40px, border-radius `--radius-lg`, min-height 420px, flex column, justify-content space-between
- Eyebrow "БЛИЖАЙШАЯ ЭКСПЕДИЦИЯ" — цвет `rgba(255,255,255,.85)`
- H3 страны — Playfair Display 700, 36px, color white
- Даты — Inter 500 15px, color `rgba(255,255,255,.9)`
- CTA — `.btn-liquid-glass`, но на этом фоне вариант "инвертированный": фон белый, текст `--orange-600` (чтобы не сливаться с градиентом карточки)

**Обычная карточка направления:**
- `.glass-01` или `--bg-elevated` (плоская, без блюра — выбрать плоскую для перфоманса, т.к. таких карточек много на экране), border-radius `--radius-md`, padding 24px
- Статус-бейдж вверху (`.badge-active` / `.badge-soon` / `.badge-completed`)
- H3 страны (20px), город/регион (`--type-meta`), margin-top auto перед бейджем в самом низу
- Hover: translateY(-6px), `--shadow-hover`

### 2.6 Почему FExperience — стеклянные карточки с фоновым изображением

**Концепция точно по референсу ("Launch Your Ambitions"):**
Три слоя в каждой карточке:
1. **Фоновое тематическое изображение** — занимает всю карточку (`object-fit: cover`)
2. **Стеклянная панель** — frosted glass поверх фото с угловыми оранжевыми дугами
3. **Текст + тег-пилюля** — живой HTML поверх стекла

Карточка №1 — featured (крупнее, полный Glass Signature с glow). Карточки №2–6 — Glass Level 01. Принцип "одна wow-точка на экран" сохраняется: у featured сильнее glow и обе угловые дуги, у остальных — только одна дуга и без glow.

**Фон секции:** `--color-surface` (#F5F2EC).

**Заголовок секции:**
```
Eyebrow: "ПОЧЕМУ FEXPERIENCE"
H2: "Шесть причин выбрать нас"  — Playfair Display 600, 40px
margin-bottom: 48px
```
...
.why-card:first-child { grid-row: span 2; }  /* featured — высокая */
```
Mobile: 1 колонка, все одинаковые, featured без span.

---

**Структура HTML каждой карточки (три слоя):**
...
<div className="why-card why-card-featured">
  ...
  <Image
    src="/images/why/networking.webp"
    alt=""
    fill
  ...
  {/* СЛОЙ 2: стекло + дуги (CSS ::before/::after) */}
  {/* реализован через CSS класса — ничего в JSX */}

  {/* СЛОЙ 3: контент поверх стекла */}
  <div className="why-card-content">
    <span className="why-card-number">01</span>
    <h3 className="why-card-title">Уникальный нетворкинг Forbes</h3>
    <p className="why-card-text">...</p>
    {/* Тег-пилюля внизу как "INSPIRING CONTENT" в референсе */}
    <div className="why-card-pill">
      <Icon size={16} />
      <span>НЕТВОРКИНГ УРОВНЯ FORBES</span>
    </div>
  </div>
</div>
```
...
**CSS — карточка №1 (Featured, Glass Signature):**

```css
.why-card-featured {
  position: relative;
  border-radius: var(--radius-lg);
  overflow: hidden;          /* ← изображение обрезается по радиусу */
  min-height: 480px;
  display: flex;
  flex-direction: column;
  justify-content: flex-end; /* контент внизу как на референсе */
}
...
.why-card-bg-image {
  ...
  z-index: 0;
  /* next/image с fill автоматически делает position: absolute inset: 0 */
}

/* Слой 2a: градиентный overlay — затемняет нижнюю часть для читаемости текста */
.why-card-featured::before {
  content: '';
  position: absolute; inset: 0;
  background: linear-gradient(
    180deg,
    rgba(255, 248, 240, .1) 0%,    /* верх — почти прозрачный */
    rgba(255, 245, 235, .55) 50%,  /* середина — тёплое стекло */
    rgba(255, 240, 225, .82) 100%  /* низ — плотное, текст читается */
  );
...
/* Слой 2b: угловые дуги с оранжевым свечением */
/* Верхний левый */
.why-card-featured .corner-tl {
  position: absolute; top: -1px; left: -1px;
  width: 80px; height: 80px;
  border-top: 2px solid var(--color-brand-500);
  border-left: 2px solid var(--color-brand-500);
  border-radius: var(--radius-lg) 0 0 0;
  filter: drop-shadow(0 0 8px rgba(255,119,34,.7));
  z-index: 3;
  pointer-events: none;
}
/* Нижний правый */
.why-card-featured .corner-br {
...
.why-card-featured {
  box-shadow: 0 0 40px rgba(255,107,44,.2), 0 16px 48px rgba(0,0,0,.1);
}
...
.why-card-title {
  font: 600 26px/1.25 'Playfair Display', serif;
  color: var(--color-text-primary);
  margin-bottom: 12px;
}
.why-card-text {
  font: 400 16px/1.65 'HelveticaNeueCyr', sans-serif;
  color: var(--color-text-secondary);
}

/* Тег-пилюля (как "INSPIRING CONTENT" в референсе) */
.why-card-pill {
  display: inline-flex;
  align-items: center;
  gap: 8px;
  ...
  margin-top: 20px;
  font: 600 12px/1 'HelveticaNeueCyr', sans-serif;
  text-transform: uppercase;
  letter-spacing: .05em;
  color: var(--color-text-primary);
}
...
**CSS — карточки №2–6 (Glass Level 01 с фоном):**
...
/* Градиентный стекло-overlay — только снизу */
.why-card::before {
  content: '';
  position: absolute; inset: 0;
  background: linear-gradient(
    180deg,
    transparent 20%,
    rgba(255,248,240,.70) 70%,
    rgba(255,245,235,.90) 100%
  );
  backdrop-filter: blur(1.5px);
  z-index: 1;
}

/* Угловая дуга — только верхний левый, без glow */
.why-card::after {
  content: '';
  position: absolute; top: 0; left: 0;
  width: 48px; height: 48px;
  border-top: 1.5px solid rgba(255,119,34,.55);
  border-left: 1.5px solid rgba(255,119,34,.55);
...
.why-card .why-card-number {
  font: 700 36px/1 'Playfair Display', serif;
  color: var(--color-brand-200);
}
...
.why-card .why-card-text {
  font: 400 14px/1.6 'HelveticaNeueCyr', sans-serif;
  color: var(--color-text-secondary);
}
```
...
**Фоновые изображения для каждой карточки — по смыслу текста:**

| № | Заголовок | Изображение | Файл |
|---|---|---|---|
| 01 | Уникальный нетворкинг Forbes | Деловое рукопожатие / группа людей в переговорах, тёплый свет | `why-01-networking.webp` |
| 02 | Медийное сопровождение | Видеосъёмка в деловой среде / экран с публикацией Forbes | `why-02-media.webp` |
| 03 | Экспансия бизнеса без купюр | Городской деловой квартал незнакомого рынка, рассвет | `why-03-expansion.webp` |
| 04 | Контакты с госструктурами | Официальная встреча в переговорной / здание со флагами | `why-04-government.webp` |
| 05 | Оценка бизнес-модели | Данные на экране / аналитик за работой с графиками | `why-05-analytics.webp` |
| 06 | Культурное погружение | Локальная архитектура / культурный контекст страны рынка | `why-06-culture.webp` |

Путь: `/public/images/why/[filename]`
Формат: `.webp`, размер 800×600px (4:3), quality 80.
Тематика: деловая среда — **не туристическая**, не стоковые улыбки в костюмах. Референс по стилю — фото с `/about` на текущем сайте.

**Промпты для генерации** — добавлены в `FExperience_AI_Image_Prompts.md` раздел 13.12.

---

**Полный список 6 карточек (текст с сайта):**

| № | Тег-пилюля | Заголовок | Текст |
|---|---|---|---|
| 01 | НЕТВОРКИНГ УРОВНЯ FORBES | Уникальный нетворкинг Forbes | Деловое общение с предпринимателями, опытными экспертами и местным бизнес-сообществом, где каждая встреча имеет практическую ценность. |
| 02 | МЕДИА FORBES | Медийное сопровождение | Появление материалов о вас и вашем бренде на информационных площадках Forbes — ваш репутационный капитал. |
| 03 | РЕАЛЬНЫЙ РЫНОК | Экспансия бизнеса без купюр | Шанс занять место там, где рынок только формируется. Риски, перспективы и инструменты, которые работают. |
| 04 | ПРЯМОЙ ДОСТУП | Контакты с госструктурами | Прямой диалог с торговыми палатами, регуляторами и госаппаратом. Без бюрократии, без посредников. |
| 05 | СТРЕСС-ТЕСТ | Оценка бизнес-модели | Обратная связь от локальных экспертов и потенциальных партнёров. Стресс-тест гипотез через призму региона. |
| 06 | КОНТЕКСТ РЫНКА | Культурное погружение | Через знакомство с местными традициями — понимание деловой среды и правил взаимодействия. |
...
Подзаголовок:
  Inter 400, 16px, --text-secondary, line-height 1.6, max-width 440px, margin-bottom 40px
...
### 2.8 Testimonials
> ⚠️ Финальный CSS `.testimonial-card` — в разделе 14.9. Здесь — данные и структура.

**Layout:** карусель через Embla Carousel (уже установлен). Карточка width 400px (desktop), gap 24px.

```tsx
// Embla для Testimonials — тот же паттерн что HeroSlider, но другие опции
...
.testimonial-card {
  ...
  position: relative;
  min-width: 400px;  /* для горизонтальной карусели */
}
```
- Декоративная кавычка: Playfair Display 80px, color `--orange-200`, position absolute, top 16px, left 24px, line-height 0, z-index 0
- Фото автора: circle 56px, border 2px solid `--orange-200`, position absolute, top -28px, right 32px
- **Тег экспедиции** (новое — с сайта): пилюля `.badge-soon`-стиль (контурная, `--text-tertiary`) с текстом "ЮАР 2025" или "Марокко 2025", position absolute, top -28px, left 32px. Высота тега 28px — выровнен по высоте с фото.
- Имя: Inter 600 15px, `--text-primary`, margin-top 16px
- Должность + компания: `--type-meta`, line-height 1.4, margin-bottom 16px
- Разделитель: 1px `--border-hairline`, margin-bottom 16px
- Текст отзыва: Inter 400 15px, `--text-secondary`, line-height 1.7, max 4 строки с `overflow: hidden` (опционально — раскрытие по кнопке "Читать полностью")

**Данные карусели (реальные, с сайта):**

| Имя | Компания | Тег | Начало цитаты |
|---|---|---|---|
| Виталий Гайдарович | Обувная фабрика Ромер, Т-софт | ЮАР 2025 | "Экспедиция FExperience — уникальная возможность..." |
| Ольга Апалькова | KIBERone | Марокко 2025 | "Бизнес-тур в Марокко с FExperience стал..." |
| Александр Адамович | ТЕРМИНАЛ | Марокко 2025 | "С Африкой можно сотрудничать, там есть где развернуться..." |
| Михаил Цыганов | Sport Master Data SIA | Марокко 2025 | "Поездка в Марокко с FExperience показала мне..." |
| Ирина Демченко | визовый центр EUROPASS | ЮАР 2025 | "Спасибо команде Forbes за такую классную возможность..." |
| Александр Николаев | ORANGE | ЮАР 2025 | "В октябре я участвовал в бизнес-экспедиции от FExperience..." |

**Навигация карусели:** точки снизу (6 штук), активная — `--orange-600` width 20px / inactive — `--text-tertiary` width 8px, border-radius pill, transition width 200ms. Стрелки — опционально, по ширине контейнера.
...
- Вопрос: Inter 500 17px, `--text-primary`, flex justify-between с плюс/минус-иконкой (24px, `--orange-600`, rotate 45deg при открытии, transition 200ms)
- Ответ: `--type-body`, padding-top 16px, max-height transition 300ms ease при раскрытии

### 2.10 Final CTA

**Фон:** `--bg-canvas` с radial-gradient декором по центру: `radial-gradient(circle at 50% 50%, rgba(255,107,44,.08), transparent 60%)`.
...
**Блок 2 — Continental nav footer:** 4 текстовые ссылки в ряд (Africa/Asia/LATAM/Russia), Inter 500 14px, gap 32px, margin-bottom `--space-6`.

**Блок 3 — 4 колонки навигации:** grid 4 колонки desktop / 2 колонки mobile, gap 32px.
- Заголовок колонки: Inter 600 13px uppercase `--text-tertiary`, margin-bottom 16px
- Ссылки: Inter 400 15px `--text-secondary`, line-height 2.2 (вертикальный gap через line-height), hover color `--orange-600`
...
### 3.2 Деловая программа (фото-блок)

**Layout:** асимметричный bento — 1 крупное фото (span 2×2) + 3 обычных (span 1×1), grid 3 колонки, gap 16px. Mobile — горизонтальный scroll-snap, карточки 80vw ширины.

**Фото:** border-radius `--radius-md`, object-fit cover, без теней (плоские, только рамка `--border-hairline` 1px опционально).

**Текстовый блок** (до или после сетки фото): H2 "Деловая программа и погружение в локальную культуру" + текст `--type-body`, max-width 640px.

**CTA после фото-блока:**
```css
.about-expeditions-cta {
  margin-top: 40px;
  display: flex;
  align-items: center;
  gap: 8px;
...
```
`.btn-text` "Смотреть все экспедиции →" → `/expeditions`. Inter 600 15px `--orange-600`, стрелка SVG 16px с hover translateX(4px).

### 3.3 Due Diligence

**Заголовок:** eyebrow + H2 "Ключевое отличие FExperience", margin-bottom 40px.

**Grid:** 3 колонки desktop, 2 tablet, 1 mobile, gap 24px.

**Карточка (страница досье):**
- `--bg-elevated`, border: 1px solid `--border-hairline`, border-radius `--radius-md`, padding 32px, min-height 200px
- Номер в правом верхнем углу: Inter 500 13px `--text-tertiary`, формат "01 / 09"
- Иконка: 32×32px, stroke `--orange-600`, stroke-width 1.5, fill none (line-icon стиль, не заливка)
- H3 (20px), margin-top 16px
- Текст пояснения (опционально, если есть) — `--type-body`, margin-top 8px

Список 9 карточек: Безопасность в стране, Деловой климат, Регуляторная среда, Особенности логистики, Реальный запрос бизнеса, Культурные барьеры, Финансовая инфраструктура, Цифровизация экономики, Стоимость выхода на рынок.
...
**Карточка:**
- `.glass-01`, border-radius `--radius-md`, padding 24px, text-align center
- Фото: circle 96px, margin 0 auto 16px
- Бейдж "Forbes Russia": `.badge-active`-подобный, но нейтральный цвет (`--bg-surface` фон, `--text-primary` текст), margin-bottom 8px
- Имя: Inter 600 17px
- Должность: `--type-meta`

Люди: Марина Матыцина, Денис Кошкин, Анастасия Никитина.
...
Подзаголовок — Inter 400, 18px, --text-secondary, max-width 560px
...
### 4.4 Сетка направлений

**Grid:** 3 колонки desktop, 2 tablet, 1 mobile, gap 24px.

**Карточка** — идентична обычной карточке из 2.5 (Featured Markets), но здесь она базовая единица сетки, не bento — все карточки одинакового размера, статус-бейдж определяет визуальный вес (см. 0.8): `Активна` — полный контраст и hover, `Скоро` — обычный контраст, без визуального "притухания", `Завершена` — opacity карточки .85, без hover-подъёма (только border-color меняется на hover).

**Ссылка карточки "Завершена" (Марокко) на директории:**
Карточка Марокко ведёт на `/expeditions/morocco` (recap-страница, шаблон 3 из раздела 5 детальной страницы). **Не `href="#"`** — как сейчас на живом сайте. Если recap-страница ещё не готова — ставить `href="/expeditions/morocco"` и делать минимальную заглушку по шаблону 3, а не битую ссылку. Визуально карточка Марокко — opacity `.85`, без hover-подъёма, бейдж `.badge-completed`, кнопка внутри — `.btn-text` "Смотреть итоги →" вместо "Подробнее →".

**Блок статистики Программа/Спикеры/Участники на странице "Завершена":**
На recap-страницах (Марокко) блок 10.5 показывается в статичном режиме — без анимации countup, без лайв-счётчика мест. Заголовок блока меняется с "ДО НАЧАЛА ЭКСПЕДИЦИИ" на "ИТОГИ ЭКСПЕДИЦИИ". Цифры — финальные фактические данные из CMS-поля `completedStats` (не `liveStats`). Если `completedStats` не заполнено — блок скрывается.

Текстовый блок-тизер внизу страницы, если остались направления без карточек (пока не запланированы конкретные даты): H3 + список названий регионов через запятую, `--type-body`, `--text-tertiary`.
...
**CTA:**
- Шаблон "Активна": `.btn-liquid` + рядом счётчик мест `"Осталось мест: 6"` (Inter 600 14px, color `rgba(255,255,255,.8)`)
...
> ⚠️ Финальный CSS карточек — в этом же разделе (5.3). Класс `.included-card-bg` — финальный здесь.

**Концепция:** grid 3×2 на десктопе вместо слайдера — все 6 элементов видны сразу, пользователь не пропускает ни один. Слайдер остаётся только на mobile (scroll-snap, карточки 80vw).

**Фон секции:** `--bg-canvas`, padding `--space-9` 0.

**Вводный текст** (над сеткой):
```
H2 "Что включено" — Playfair Display 600 36px
Подзаголовок — Inter 400 17px --text-secondary, max-width 560px
"Мы полностью берём на себя организацию экспедиции и решение всех
операционных вопросов. Вы занимаетесь бизнесом и нетворкингом —
обо всём остальном позаботится команда FExperience."
margin-bottom: 40px
```
...
/* Карточка №1 (Медийное сопровождение) — featured, span 2 колонки */
.included-card:first-child {
  grid-column: span 2;
}
```
Tablet: 2 колонки, первая карточка не span. Mobile: scroll-snap горизонтальный.
...
.included-card:hover .included-card-bg {
...
/* Стеклянная подложка с подписью */
.included-card-label {
  position: absolute;
...
.included-card-label-icon {
  width: 18px; height: 18px;
  stroke: rgba(255,255,255,.9); fill: none; flex-shrink: 0;
}
.included-card-label-text {
  font: 600 15px Inter;
  color: rgba(255,255,255,.95);
  line-height: 1.3;
}
...
**Карточка №1 (featured — Медийное сопровождение):**
```css
.included-card:first-child {
  min-height: 320px;   /* выше остальных */
}
/* Угловые дуги Glass Signature — только на этой карточке */
.included-card:first-child::after {
  content: '';
  position: absolute;
  bottom: 72px; right: -1px;   /* над label, правый нижний угол */
  width: 60px; height: 60px;
  border-bottom: 2px solid var(--orange-500);
  border-right: 2px solid var(--orange-500);
  border-radius: 0 0 var(--radius-md) 0;
  filter: drop-shadow(0 0 5px rgba(255,119,34,.6));
  z-index: 3;
}
```
...
| № | Подпись | Иконка |
|---|---|---|
| 1 | Медийное сопровождение | play-circle / film |
| 2 | Бизнес-сессии, нетворкинг | users |
| 3 | Культурные мероприятия | map-pin |
| 4 | Проживание в отеле 5★ | star |
| 5 | Трансферы вип-класса | car |
| 6 | Питание | coffee / utensils |

Фоновые фото — существующие с сайта (`included-bg-1.webp` … `included-bg-6.webp`). Дополнительные overlay-фото (`included-south-africa-top-1.webp` … `top-6.webp`) — в новом дизайне убираются, они создавали двойной слой; одного богатого фото-фона достаточно.
...
**Фон секции программы:** `--bg-canvas`, padding `--space-10` 0.

**Layout:** двухколоночный flex, gap 40px.
- Левая колонка (width 80px, flex-shrink 0): номера дней + вертикальная линия
- Правая колонка (flex 1): карточки дней
...
/* Анимация появления — через CSS fade-up класс, не Framer Motion */
.timeline-card.fade-up { /* наследует из globals.css */ }
.timeline-card.fade-up.delay-1 { transition-delay: 80ms; }
.timeline-card.fade-up.delay-2 { transition-delay: 160ms; }
/* и т.д. по индексу дня */

/* Активная карточка — лёгкий левый border-accent */
...
Текст описания — Inter 400 15px --text-secondary line-height 1.7
```
...
**Grid:** 2 колонки desktop, 1 колонка mobile, gap 16px.
...
**Grid:** 3 колонки desktop, 1 mobile, gap 20px.
...
/* Тонкий градиентный фон — фирменный, но очень приглушённый */
.forbes-expert-card::before {
  content: '';
  position: absolute; inset: 0;
  background: var(--gradient-signature);
  opacity: .04;   /* почти невидим, но создаёт тёплый оттенок */
  pointer-events: none;
}
```
...
.forbes-expert-photo {
  ...
  border-radius: var(--radius-md);  /* квадратное с радиусом, не круглое */
  overflow: hidden;
  margin: 0 auto 16px;
  position: relative;
}
...
Eyebrow: "ДО НАЧАЛА ЭКСПЕДИЦИИ"
  Inter 600 13px uppercase --orange-600 letter-spacing .08em
  margin-bottom: 24px
...
Подпись под счётчиком: "ЮАР · 15–21 ноября 2026"
  Inter 500 14px --text-tertiary, margin-top 20px
```
...
**Дата экспедиции** для счётчика — из CMS поля `startDate` (ISO string). Агент не хардкодит дату. Когда `startDate` в прошлом — блок таймера скрывается (`display: none`), его место занимает статичная строка "Экспедиция началась".
...
```
Select "Выберите экспедицию" — динамический из CMS (см. 10.7)
Input "Ваше имя" ★
Input "Телефон" ★
Input "E-mail" ★
...
- **Отзывы:** та же карточка, что в 2.8, но не карусель, а статичная сетка 3 колонки (все отзывы этой экспедиции видны сразу).
...
Подзаголовок — Inter 400 18px --text-secondary, max-width 480px
...
Featured-статья — последняя опубликованная из CMS (`orderBy: publishedAt desc, limit: 1`), не хардкодить.
...
**Grid:** 3 колонки desktop, 2 tablet, 1 mobile, gap 32px (журнальный простор, больше чем обычные 24px).

**Карточка статьи:**
- Обложка: aspect-ratio 4:3, border-radius `--radius-md`, object-fit cover, без рамки/тени
- Hover: обложка scale(1.03) внутри overflow:hidden контейнера, 400ms ease
- Дата + категория: `--type-meta`, margin-top 16px, flex gap 8px, разделены точкой "·"
- H3 заголовка: 20px Playfair Display 600, margin-top 8px
- Decorative line под заголовком при hover: width 0→32px `--orange-600` height 2px, transition 300ms

**Пустое состояние фильтра** (когда статей по выбранной категории нет):
```css
.articles-empty {
...
```
```
Иконка: SVG компаса 48px, stroke --orange-300
H3 "Статей по этой теме пока нет"
  Playfair Display 600, 24px, --text-primary, margin-top 16px
Подтекст: Inter 400, 15px, --text-secondary
  "Выберите другую категорию или читайте все материалы."
btn-outline "Показать все статьи" — сбрасывает активный фильтр, margin-top 20px
```
...
```css
.article-body { font: 400 18px/1.8 Inter; color: var(--text-primary); }
...
Все горизонтальные grid → либо stack (vertical), либо horizontal scroll-snap
  (scroll-snap используется там, где карточек > 4 и они однотипные:
   Continental Nav, фото-bento About, карусель Testimonials)
...
### Финальный порядок и названия Hero-слайдов

Hero содержит три слайда в следующем порядке:

| № | Слайд | Точный заголовок |
|---|---|---|
| 1 | Индия | **Деловой ужин с Forbes в Индии** |
| 2 | ЮАР | **Бизнес-экспедиция в ЮАР** |
| 3 | Вьетнам | **Бизнес-экспедиция во Вьетнам** |

Обязательные правила:

- Это утверждённые тексты Hero.
- Агент не должен самостоятельно сокращать, переформулировать, переставлять или генерировать названия.
- Нельзя генерировать заголовок по шаблону `Бизнес — + ЭКСПЕДИЦИЯ + страна`.
- Нельзя использовать только название страны вместо полного заголовка.
- Нельзя самостоятельно менять порядок слайдов.
- Слайд 1 — `Деловой ужин с Forbes в Индии` — является временным контентом до сентября 2026 года.
- После сентября 2026 года первый слайд заменяется на актуальную бизнес-экспедицию, но общая Hero-композиция, glass/video split, типографическая логика и CTA не изменяются.
- До отдельного изменения спецификации агент не должен самостоятельно менять текст первого слайда.
- Не создавать CMS, API или новую систему управления контентом.
...
<article className="hero-slide">
  ...
  <div className="hero-slide__glass glass-02"></div>
  ...
  <div className="hero-slide__title-layer">
    <h1 className="hero-slide__title">
      <span className="hero-slide__title-glass">
        ...
        <span className="hero-slide__title-primary">
          {expedition.heroTitlePrimary}
        </span>
      ...
      <span className="hero-slide__title-open">
        {expedition.heroTitleOpen}
      </span>

      <span className="hero-slide__title-country">
        {expedition.heroTitleCountry}
      </span>
    </h1>
    ...
    <div className="hero-slide__cta-group">
      ...
      <a
        className="hero-slide__details btn-liquid btn-liquid--secondary"
...
### Важное правило для Hero-текста

Hero не генерирует заголовок из `country`, `slug` или другого поля автоматически.

Для экспедиционных слайдов используются заранее определённые значения:
...
Для временного слайда Индии точный текст:

```text
Деловой ужин с Forbes в Индии
```

Не добавлять к нему слова `Бизнес —` или `ЭКСПЕДИЦИЯ`, которых нет в утверждённом названии.

Если текущая структура `src/data/expeditions.ts` не позволяет сохранить точный Hero-title отдельно от названия направления, добавить только минимальные поля для Hero-текста, не меняя существующую архитектуру данных.
...
.hero-slide__title {
  ...
  font: 700 clamp(88px, 7.2vw, 140px)/.86 'Playfair Display', serif;
...
/* CTA — одна компактная горизонтальная группа */
...
4. Glass занимает ровно `40%` ширины и `100%` высоты Hero на desktop.
5. Glass не является floating-card.
6. Desktop glass не имеет `border-radius`.
7. Видео визуально продолжается за glass.
8. Внутри glass нет декоративного `FEXPERIENCE`.
9. Нет `FORBES × БИЗНЕС-ЭКСПЕДИЦИИ`.
10. `Бизнес —` — крупная часть H1 в glass для экспедиционных слайдов.
11. `ЭКСПЕДИЦИЯ` — продолжение того же H1 на открытом фоне.
12. `В ЮАР` / `Во Вьетнам` — вторая строка того же H1 со смещением вправо.
13. `с Forbes` — вертикально слева от `Б`, направление чтения снизу вверх.
14. Дата — непосредственно под первой частью H1.
15. Слайд 1: `Деловой ужин с Forbes в Индии`.
16. Слайд 2: `Бизнес-экспедиция в ЮАР`.
17. Слайд 3: `Бизнес-экспедиция во Вьетнам`.
18. Тексты слайдов не генерируются автоматически из страны и не переформулируются.
19. Первый слайд является временным контентом до сентября 2026 года.
20. CTA находятся в одной компактной горизонтальной группе.
21. CTA не разнесены к противоположным сторонам экрана.
22. `Стать участником` использует primary Liquid Glass.
23. `Подробнее` использует secondary Liquid Glass.
24. Три тезиса — pills в нижней части Hero.
25. Pills не перекрывают H1 и CTA.
26. Источник данных — `src/data/expeditions.ts`.
27. CMS отсутствует.
28. Не использовать Sanity, Strapi, Contentful, внешний API или другую CMS-механику.
29. Active Hero использует `heroVideo` на desktop.
30. На mobile используется `heroPoster`.
31. Смена slide меняет video, направление, дату, CTA и pills текущей экспедиции.
32. Swipe реализуется Embla, без собственных touch-handlers.
33. Не добавлять новые визуальные элементы, которых нет в данной спецификации.
34. Не менять тексты слайдов без отдельного указания.

---

## Liquid Glass — secondary CTA

В существующем разделе `14.1` использовать текущую систему `.btn-liquid` и добавить только secondary-вариант:
...
Не создавать новую независимую button-system.

---

## CMS Lock

CMS в проекте отсутствует.

Все данные Hero берутся из существующего локального `src/data/expeditions.ts`.
...
Если в старом файле есть закомментированный блок вида:
...
его удалить, чтобы AI-агент не воспринимал CMS как допустимую часть архитектуры.

---

## Финальный визуальный LOCK

Правильный Hero должен восприниматься как:

**fullscreen video + вертикальное glass-поле 40% + крупная editorial typography, пересекающая границу glass/video + вертикальное `с Forbes` рядом с `Б` + компактная группа из двух Liquid Glass CTA.**

Не использовать:

- floating glass-card;
- огромный обычный text-overlay;
- отдельную карточку с текстом;
- две далеко разнесённые кнопки;
- обычную flat-кнопку `Подробнее`;
- автоматически сгенерированное `Бизнес — ЭКСПЕДИЦИЯ + страна`;
- отсутствующий `с Forbes`.

**Никаких других изменений в spec или проекте по этой задаче не делать.**
...
**Содержимое:**
- Текст: Inter 14px `--text-secondary`, "Мы используем cookies. Продолжая использование сайта, вы соглашаетесь с обработкой данных." Ссылка "Подробнее о политике" → `/privacy`, цвет `--orange-600`.
...
- Заголовок: "ПРОГРАММА", Inter 600 13px, letter-spacing .08em, color `rgba(255,255,255,.5)`
- 2 счётчика рядом:
  - Цифра: Playfair Display 700, 64px, color `var(--orange-600)`
  - Подпись: Inter 400 14px, color `rgba(255,255,255,.7)`, "деловая часть" / "культурное погружение"
- Значения (из CMS для текущей featured экспедиции, например ЮАР): "70% деловая часть / 30% культурное погружение"
...
- Заголовок: "УЧАСТНИКИ"
- Крупная строка: ">N МЛРД РУБ — годовой оборот", N — из CMS
- Подстрока: "НЕТВОРКИНГ на уровне первых лиц", Inter 600 15px, `--orange-400`
...
Выпадающий список в форме заявки (на странице детальной экспедиции и, если есть, на главной) **не хардкодится**. Он формируется динамически из CMS: все записи с полем `status: "активна"`, отсортированные по дате по возрастанию.
...
```typescript
// src/data/expeditions.ts — данные уже здесь, не в CMS
import { expeditions } from '@/data/expeditions'
...
- Тело: Inter 400 17px, line-height 1.75, `--text-primary`
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      description: expedition.shortDescription,
      images: [{
        url: expedition.ogImage, // поле в CMS: 1200×630px
        width: 1200,
        height: 630,
...
**Таблица OG-изображений по страницам** (нужно подготовить в CMS):
...
| `/expeditions/[slug]` | из CMS поля `ogImage` | 1200×630 |
...
**Форматы в CMS:** принимать только `.webp` или `.jpg`. PNG — только для логотипов с прозрачностью. Видео — `.webm` с `.mp4` fallback. Poster-изображение для видео — обязательно (загружается до начала воспроизведения).
...
[Центр страницы, max-width 640px, text-align center]
  ...
  Подзаголовок
  Inter 400, 17px, --text-secondary, line-height 1.6
...
**Источник:** реальный отзыв участника именно этой экспедиции из поля `testimonials` в CMS. Для ЮАР — любой из: Гайдарович, Демченко, Николаев. Поле в CMS: `pullQuote: { text, author, company }`.
...
[Атрибуция — через 24px]
  Inter 600, 15px, --text-primary   → имя автора
  Inter 400, 13px, --text-tertiary  → должность / компания
...
**Контент:** `"6"` или `"7"` — число дней из поля `duration` в CMS. Не слово, только цифра.
...
### 11.8 Hover-состояние карточек экспертов

Карточки экспертов статичны — добавить минимальный, но осознанный hover.
...
.expert-card:hover .expert-photo {
  /* лёгкий зум фото внутри карточки */
  transform: scale(1.04);
  transition: transform .4s var(--ease);
}
.expert-photo {
...
Не добавлять при hover появление имени/должности — они и так видны постоянно (зафиксировано в разделе 5.5). Hover — только физическое поднятие карточки + мягкий зум фото.
...
2. International business delegation visiting local [industry] company,
...
.glass-01 {
  ...
  border: 1px solid rgba(200, 230, 255, .7);
...
.glass-01::before {
  ...
  background: linear-gradient(
    ...
    rgba(248, 252, 255, .25) 65%,    /* холодная дымка начинается */
    rgba(240, 248, 255, .55) 85%,
    rgba(235, 245, 255, .70) 100%    /* максимум внизу */
  );
  pointer-events: none;
  z-index: 0;
}
...
.glass-01:hover {
  transform: translateY(-5px);
  box-shadow:
    0 24px 64px rgba(180, 150, 120, .15),
...
subtle blue-white coloring — cold, pure, clean,
...
### 14.9 Testimonial карточка — стекло с кавычкой
...
.testimonial-quote-mark {
  ...
  top: 12px;
  left: 24px;
  font: 700 96px/0.8 'Playfair Display', serif;
  color: var(--orange-200);
  pointer-events: none;
  user-select: none;
  z-index: 0;
  /* Градиентное затухание снизу */
  -webkit-mask-image: linear-gradient(180deg, black 0%, transparent 100%);
  mask-image: linear-gradient(180deg, black 0%, transparent 100%);
}

/* Тег экспедиции в верхнем левом углу */
.testimonial-tag {
  position: absolute;
  top: -14px;
  left: 24px;
...
/* Фото автора — выступает из карточки */
.testimonial-photo {
  position: absolute;
  top: -24px;
  right: 24px;
...
### 14.10 Карточки "Почему FExperience" (Why-блок)

**Featured карточка №1** — самая сложная, полная реализация:
...
.why-card-featured {
  ...
  isolation: isolate;
  overflow: visible; /* дуги выходят за рамки */

  box-shadow:
    0 0 0 1px rgba(255, 119, 34, .15),
    0 0 40px rgba(255, 107, 44, .12),
    0 0 100px rgba(255, 107, 44, .06),
    0 24px 80px rgba(0, 0, 0, .08),
    inset 0 1px 0 rgba(255, 255, 255, .95);
}

/* Внутреннее рассеянное свечение сверху */
.why-card-featured::before {
  content: '';
  position: absolute;
  top: 0; left: 0; right: 0;
...
```tsx
// Угловые дуги через SVG (переиспользуется из 14.3)
// Номер карточки — декоративный watermark
function WatermarkNumber({ number }: { number: string }) {
  return (
    <div style={{
      position: 'absolute',
      top: 24, right: 32,
      font: '700 72px/1 "Playfair Display", serif',
      color: 'var(--orange-200)',
      userSelect: 'none',
      pointerEvents: 'none',
      zIndex: 0,
...
**Карточки №2–6** — стандартные с упрощёнными дугами:
...
.why-card {
  ...
  /* Тонкий top border как световая грань стекла */
...
/* Угловая дуга — только верхний левый, упрощённая без glow */
.why-card::before {
  content: '';
  position: absolute;
  top: 0; left: 0;
  width: 48px; height: 48px;
  border-top: 1.5px solid rgba(255, 119, 34, .5);
  border-left: 1.5px solid rgba(255, 119, 34, .5);
  border-radius: var(--radius-md) 0 0 0;
  pointer-events: none;
}
```
...
### 14.12 Правило применения — сводная таблица

| Элемент | CSS-реализация | PNG нейросеть |
|---|---|---|
| Кнопка Liquid Glass | ✅ раздел 14.1 | ❌ не нужна |
| Glass Level 01/02 | ✅ раздел 14.2 | ❌ не нужна |
| Glass Signature карточка | ✅ раздел 14.3 | ❌ не нужна |
| Featured градиент карточка | ✅ раздел 14.4 | ❌ не нужна |
| Hero теги-пилюли | ✅ раздел 14.5 | ❌ не нужна |
| Таймер блоки | ✅ раздел 14.6 | ❌ не нужна |
| Pulse-точка карты | ✅ раздел 14.7 | ❌ не нужна |
| Progress Bar | ✅ раздел 14.8 | ❌ не нужна |
| Testimonial карточка | ✅ раздел 14.9 | ❌ не нужна |
| Why-карточки (6 штук) | ✅ раздел 14.10 | ❌ не нужна |
| Форма заявки панель | ✅ раздел 14.11 | ❌ не нужна |
| Иконки Due Diligence | ❌ код не справится | ✅ PNG прозрачный фон |
| Фоновые фото (hero/included/about) | ❌ | ✅ .webp из раздела 2-5 промптов |
| SVG карты континентов | ✅ inline SVG | ❌ Natural Earth Data |
  ...
  return (
    <div style={{ position: 'relative', maxWidth: 800, margin: '0 auto' }}>
      ...
      <div className="glass-01" style={{
        ...
        <span style={{ font: '400 12px Inter', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '.06em' }}>
...
### 14.12 Правило применения — сводная таблица (обновлено)
...
| Testimonial карточка | ✅ CSS | раздел 14.9 |
| Why-карточки (6 штук) | ✅ CSS + SVG угловые дуги | раздел 14.10 |
...
export default function ExpeditionError({
  ...
  return (
    <div style={{
      ...
      <p style={{ font: '400 16px Inter', color: 'var(--text-secondary)' }}>
